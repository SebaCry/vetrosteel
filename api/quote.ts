/**
 * POST /api/quote — Vercel Function. Vercel deploys any file in the root api/
 * directory; no vercel.json entry is needed.
 *
 * This file must stay free of relative imports. Vercel compiles each function
 * file to ESM on its own, and an extensionless import (`'../src/...'`) then
 * fails to resolve at load time — every request died with
 * FUNCTION_INVOCATION_FAILED. `npm run test:quote` checks LINES against
 * src/data/verticals.ts so the two cannot drift.
 *
...
/** Line-of-work slug → label. Mirrors src/data/verticals.ts (checked by test:quote). */
...
function validate(data: Payload) {
  ...
  if (!Object.hasOwn(LINES, vertical)) fields.vertical = 'Pick the line of work this falls under.';
...
export async function handleQuote(request: Request, env: QuoteEnv): Promise<Response> {
  ...
  const line = LINES[clean.vertical];
